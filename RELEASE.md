...
This project ships Linux release assets for Meeting Recorder v0.2.0. Do not publish from automation in this repository without an explicit release decision; the builder only creates local artifacts.

## Assets

The release builder creates:

- `dist/meeting-recorder-app-0.2.0-linux-source-installer.tar.gz`
- `dist/meeting-recorder-app_0.2.0_all.deb` when `dpkg-deb` is installed
- `dist/SHA256SUMS`
...
TMP_HOME=$(mktemp -d)
TMP_PREFIX=$(mktemp -d)
tar -xzf dist/meeting-recorder-app-0.2.0-linux-source-installer.tar.gz -C "$TMP_HOME"
(cd "$TMP_HOME/meeting-recorder-app-0.2.0" && HOME="$TMP_HOME" PREFIX="$TMP_PREFIX" ./install.sh)
HOME="$TMP_HOME" "$TMP_PREFIX/bin/meeting-recorder" --help
HOME="$TMP_HOME" "$TMP_PREFIX/bin/meeting-recorder" doctor || true
HOME="$TMP_HOME" PREFIX="$TMP_PREFIX" "$TMP_PREFIX/opt/meeting-recorder-app-0.2.0/uninstall.sh"
```

Debian package inspection, when available:

```bash
dpkg-deb --info dist/meeting-recorder-app_0.2.0_all.deb
dpkg-deb --contents dist/meeting-recorder-app_0.2.0_all.deb
```
...
4. Create and push tag `v0.2.0` only after approval.
5. Upload the source installer, Debian package when built, and SHA256SUMS to the GitHub release.

This Round 3 implementation intentionally does not publish or push release assets.
